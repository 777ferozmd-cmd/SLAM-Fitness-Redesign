...
export default function Footer() {
  ...
  return (
    <footer className="bg-[#141414] border-t-2 border-[#FF1A1A]">
          ...
          <div className="flex flex-col space-y-6">
            <Link href="/" className="inline-block">
              <span className="text-3xl font-extrabold text-white tracking-tight">
                SLAM<span className="text-[#FF1A1A]">.</span>
              </span>
            </Link>
            ...
            <div className="flex space-x-4 pt-2">
              <a 
                ...
                className="text-[#B0B0B0] hover:text-[#FF1A1A] transition-colors"
            ...
            <ul className="flex flex-col space-y-3">
              {NAV_LINKS.filter(link => !link.subLinks).map((link) => (
                <li key={link.label}>
                  <Link 
                    href={link.href}
                    className="text-[#B0B0B0] hover:text-[#FF1A1A] transition-colors inline-block"
                  >
          ...
          <div className="flex flex-col space-y-6">
            ...
            <div className="flex flex-col space-y-4 text-[#B0B0B0]">
              <div className="flex items-start space-x-3">
                <Phone className="w-5 h-5 text-[#FF1A1A] shrink-0 mt-0.5" />
                <div className="flex flex-col space-y-1">
                  <span className="font-medium text-white">{CONTACT.contactPerson}</span>
                  <a href={`tel:${CONTACT.customerCare.replace(/\s+/g, '')}`} className="hover:text-[#FF1A1A] transition-colors">
                    {CONTACT.customerCare}
                  </a>
                  <a href={`tel:${CONTACT.corporatePhone.replace(/\s+/g, '')}`} className="hover:text-[#FF1A1A] transition-colors">
              ...
              <div className="flex items-start space-x-3">
                <Mail className="w-5 h-5 text-[#FF1A1A] shrink-0 mt-0.5" />
                <a href={`mailto:${CONTACT.customerEmail}`} className="hover:text-[#FF1A1A] transition-colors break-all">
                  {CONTACT.customerEmail}
                </a>
              </div>

              <div className="flex items-start space-x-3">
                <MapPin className="w-5 h-5 text-[#FF1A1A] shrink-0 mt-0.5" />
      ...
      <div className="border-t border-[#2A2A2A]">
        <div className="max-w-[1200px] mx-auto px-6 py-6 lg:px-20 flex flex-col md:flex-row justify-center items-center text-sm text-[#6B6B6B]">
          <p>© {currentYear} SLAM Fitness Studio. All Rights Reserved.</p>
        </div>
      </div>
    </footer>
  );
